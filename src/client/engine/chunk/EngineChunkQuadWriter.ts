...
export default {
	drawQuad(vertexArray: geometrics.VertexArrayType, quadId: number, blockPos: BlockPos, side: geometrics.SideType, uvs: Array<number>, brightnesses: Array<number>) {
	...
	clearQuad(vertexArray: geometrics.VertexArrayType, quadId: number) {
		let cursor = quadId * geometrics.quadVertexByteSize
		for (let i = 0; i < 4; i += 1) {
	...
	updateQuadAO(vertexArray: geometrics.VertexArrayType, quadId: number, blockPos: BlockPos, side: geometrics.SideType, uvs: Array<number>, brightnesses: Array<number>): boolean {
		let changed = false
		// TODO: optimize?
		changed = true
